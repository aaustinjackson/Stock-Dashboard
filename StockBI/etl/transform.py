...
def load_and_transform(ticker):
    ...
    # Indicators (compute only if column exists)
...
def plot_price_with_ma(df, ticker):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(plots_path, f"{ticker}_ma.png"))
    plt.close()
...
def plot_returns_distribution(df, ticker):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(plots_path, f"{ticker}_returns.png"))
    plt.close()
...
def plot_volatility(df, ticker):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(plots_path, f"{ticker}_volatility.png"))
    plt.close()
...
def plot_cumulative_returns(df, ticker):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(plots_path, f"{ticker}_cumulative.png"))
    plt.close()

# ---------------------------
# Run for top 10 tickers
# ---------------------------
if __name__ == "__main__":
    top10_tickers = ["AAPL", "MSFT", "AMZN", "NVDA", "GOOGL", "META", "BRK-B", "UNH", "TSLA", "JPM"]
